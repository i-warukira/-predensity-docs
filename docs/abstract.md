...
[Quality](prediction-quality.md) is scored on two dimensions: **sharpness** (how narrow the range) and **lead time** (how early the prediction). These combine into a kernel weight $$a_u$$ that determines each winner's share of the [payout pool](payouts.md).
...
| **Information-rich** | Rewards precision, producing a genuine [density signal](probability-map.md) |
| **Exit liquidity** | DPM lets early, correct forecasters exit before resolution via virtual surplus |
...
The system starts with crypto price forecasting and expands to [sports, politics, finance, and technology](markets.md).
