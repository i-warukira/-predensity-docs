...
Band probabilities (the confidence contour thresholds at 25% / 50% / 75%) slice the continuous density into interpretable regions.
...
| P25 to P75 band | Middle 50% of stake-weighted opinion: tighter = stronger consensus |
...
The chart is read-only and does not feed into the quality score. It exists to show you where the crowd's kernel mass is concentrated before you decide where to place yours.
