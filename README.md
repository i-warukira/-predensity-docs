# Abstract

Predensity is a decentralized prediction market that rewards *precision* and *foresight* — not just correctness.

Traders pick a price range and a time horizon, stake USDC, and earn based on how hard their prediction was to make. [Quality](docs/prediction-quality.md) is scored on two dimensions: **sharpness** (how narrow the range) and **lead time** (how early the prediction). These combine into a weight that determines each winner's share of the [payout pool](docs/payouts.md).

The system starts with crypto price forecasting and expands to [sports, politics, finance, and technology](docs/markets.md). The mechanics are the same across all categories: *predict a range, commit a stake, get rewarded for quality*.
